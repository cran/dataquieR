...
util_acc_varcomp <-
  ...
  icc_output <- apply(cbind(rvs, group_vars), 1, function(row) {
    ...
    if (length(critical_levels) > 0) {
      util_message("Levels %s were excluded due to fewer than %d observations.",
  ...
  SummaryData$Median.Class.Size <-
    util_round_to_decimal_places(SummaryData$Median.Class.Size, 1)


  return(list(SummaryTable = SummaryTable,
              SummaryData = SummaryData,
              ScalarValue_max_icc = max_icc,
