test_that("util_app_sl works", {
  ...
  meta_data <- prep_create_meta(
    ...
    MISSING_LIST = "",
    SOFT_LIMITS = c("[5.5;10.25]", "")
  )

  result0 <- util_app_sl(meta_data, dta = c(0, 0))
  ...
  result1 <- util_app_sl(meta_data, dta = c(1,1))
  ...
  meta_data <- prep_create_meta(
    ...
    DATA_TYPE = c(DATA_TYPES$INTEGER, DATA_TYPES$INTEGER),
    MISSING_LIST = "",
    SOFT_LIMITS = c("[5;25]", "[1;100]")
  )

  result2 <- util_app_sl(meta_data, dta = c(1,1))
  ...
  result3 <- util_app_sl(meta_data, dta = c(1,1))
